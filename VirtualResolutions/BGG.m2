...
--------------------
print("this is a version of BGG made by DE on 16/04/18")
--------------------
symExt = method()
symExt(Matrix, PolynomialRing) := Matrix => (m,E) ->(
     ...
     ans1 := transpose map(E^{(rank source ans):(degrees E^1)_0}, ,transpose ans);
    ans1
...
I = ideal"ax,by"
M = S^1/I
E = ZZ/101[A,B,X,Y,SkewCommutative =>true]
reg = {1}
p = presentation truncate(reg, M)
 symExt (p, E)
*-
-*
S = ZZ/101[a,b,x,y,Degrees => {2:{1,0},2:{0,1}}]
I = ideal"ax,by"
M = S^1/I
E = ZZ/101[A,B,X,Y,SkewCommutative =>true, Degrees => {2:{1,0},2:{0,1}}]
reg = {1,1}
p = presentation truncate(reg, M)
 symExt (p, E)
*-
...
exteriorTateResolution = method(TypicalValue => ChainComplex)


exteriorTateResolution(Module, PolynomialRing, List, ZZ) := ChainComplex => 
(M,E,reg,len)->(
    --in this version, reg is a multi-degree that should be inside
    --the multi-graded regularity of coker m.
    --Returns a resolution of a multi-homomogeneous
    --module over the exterior algebra generated in multi-degree -reg
     S := ring M;
     mt  := presentation truncate(reg,M);
     ans   := symExt(mt,E);
     F := res(coker ans, LengthLimit=>max(1,len));
     F)

exteriorTateResolution(Matrix, PolynomialRing, ZZ, ZZ) := ChainComplex => (m,E,loDeg,hiDeg)->(
    --this version works only in the singly graded case, computes the regularity.
     M := coker m;
     reg := regularity M;
     bnd := max(reg+1,hiDeg-1);
     mt  := presentation truncate(bnd,M);
     ans   := symExt(mt,E);
     res(coker ans, LengthLimit=>max(1,bnd-loDeg+1)))
...
check BGG
*-
tateToCohomTable = method()
tateToCohomTable ChainComplex := T -> (
     C := betti T;
...
cohomologyTable(CoherentSheaf, ZZ, ZZ) := CohomologyTally => (F,lo,hi) -> (
     ...
     T := exteriorTateResolution(presentation M, E, lo, hi);
--          if o.Natural == true then T = T**(ring T)^{hi-1};
     tateToCohomTable (T**(ring T)^{hi-1})
     )

cohomologyTable(Matrix, PolynomialRing, ZZ, ZZ) := CohomologyTally => (m,E,lo,hi) -> (
     T := exteriorTateResolution(m, E, lo, hi);
     tateToCohomTable (T**(ring T)^{hi-1})
     )
...
doc ///
   Key
    exteriorTateResolution
    (exteriorTateResolution, Matrix,PolynomialRing,ZZ,ZZ)
    (exteriorTateResolution, Module,PolynomialRing,List,ZZ)
   Headline
    finite piece of the Tate resolution of a module
   Usage
    T = exteriorTateResolution(m, E, loDeg, hiDeg)
    T = exteriorTateResolution(M, E, multiDeg, len)    
   Inputs
    m:Matrix
     presentation of M
    E:PolynomialRing
     exterior algebra whose generators correspond to those of ring M
    loDeg:ZZ
    hiDeg:ZZ
     bounds for the length of the resolution
    multiDeg:List
     a multi-degree inside the regularity set of M
    len:ZZ
     length of resolution to compute
   Outputs
    T:ChainComplex
   Description
    Text
     This function takes a module, or a linear matrix matrix, 
     over a commutative polynomial ring, possibly
     with multi-grading, and returns a resolution over the  dual exterior algebra.
     In the form where a linear matrix is given, the polynomial ring must have
     only a single grading, and the routine computes the regularity of M = coker m;
     In the case where a module is given, the multi-degree should be inside the
     regularity of the module, or at least the truncation of the module at this degree
     should have linear presentation.
     
     Here is an example of the singly graded situation
    Example
     S = ZZ/32003[x_0..x_2]; 
     E = ZZ/32003[e_0..e_2, SkewCommutative=>true];
     m = matrix{{x_0,x_1}};
     regularity coker m
     T = exteriorTateResolution(m,E,-2,4)
     betti T
     T.dd_1
    Text
     And here is an example of the module form
    Example
     S = ZZ/32003[x_0..x_1,y_0,y_1, Degrees =>{2:{0,1},2:{1,0}}]; 
     E = ZZ/32003[X_0..X_1,Y_0..Y_1, SkewCommutative=>true, Degrees => degrees S];
     M = S^1/ideal(x_0^2*y_0^3)
     reg = {2,3}
     T = exteriorTateResolution(M,E,reg,4)
     betti T
   Caveat
   SeeAlso
    symExt
...
document { 
     ...
     EXAMPLE lines ///
	  ...
	  m  = koszul (3, vars S); 
	  regularity coker m 
	  betti exteriorTateResolution(m,E,-6,2)
     ...
     ///, 	  
     SeeAlso => {symExt, exteriorTateResolution}
     }
...
p = presentation truncate(reg, M)
symExt (p, E)
F = exteriorTateResolution(M,E,reg,5)
betti F
assert (rank F_4 == 6)
///
...
TEST /// 
	  ...
	  apply(keys C, p -> rank HH^(p#0)(F(p#1)))

          C = cohomologyTable(F,-5,10)
	  assert(values C == apply(keys C, p -> rank HH^(p#0)(F(p#1))))
	  apply(keys C, p -> rank HH^(p#0)(F(p#1)))
///
