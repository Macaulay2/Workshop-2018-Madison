...
-*
Copyright (C) 2018 Thomas Baath, Dylan Peifer, David Swinarski
...
comprehensiveGroebnerSystem(List, List, List) := List => (E, N, F) -> (
	...
	if isConsistent(Gr, prod(N, {h})) then (
	    CGS = CGS | {(Gr, prod(N, {h}), Gm)};
	    );
	CGS = CGS | flatten for i to #H-1 list (
...
comprehensiveGroebnerBasis(List) := List => opts -> (F) -> (
    ...
    R := coefficientRing ring first F;
    comprehensiveGroebnerBasis({}, {1_R}, F, Strategy => opts.Strategy)
    )
comprehensiveGroebnerBasis(List, List, List) := List => opts -> (E, N, F) -> (
	...
	Rpot := newRing(ring first F, MonomialOrder => {Position => Down});
	Fpot := apply(F, f -> sub(f, Rpot));
	Epot := apply(E, e -> sub(e, Rpot));
	M := image ((matrix {Fpot} || matrix {toList(#Fpot:0)}) | (matrix {Epot} || matrix {-Epot}));
	CGBMod(F, E, M)
	)
    else
...
CGBMod(List, List, List) := List => (E, N, F) -> (
    ...
    -- get the new pairs using Groebner basis of module
    -- pG is "pairs G", the bold G in KSW
    R := newRing(kUX, MonomialOrder => {Position => Down});
    Fpot := apply(F, f -> sub(f, R));
    Epot := apply(E, e -> sub(e, R));
    M := image ((matrix {Fpot} || matrix {toList(#Fpot:0)}) | (matrix {Epot} || matrix {-Epot}));
    pG := select(entries transpose gens gb M, pg -> pg#0 != 0);
    pG = apply(pG, pg -> {sub(pg#0, kUX), sub(pg#1, kUX)});

    G := for pg in pG list pg#0;

    -- if the ideal is <1> then we are done
    for pg in pG do (
	if pg#0 == 1 then return {(E, N, {pg})};
	);

    -- find all elements in G that are actually in k[U]
    pGr := select(pG, pg -> (degree pg#0)#0 == 0);
    Gr := apply(pGr, pg -> sub(pg#0, kU));
	...
	PGB = PGB | flatten for i to #H-1 list (
	                        CGBMod(first entries gens gb ideal append(Gr, H#i),
				       prod(N, {product take(H, i)}),
				       apply(select(pG, pg -> (degree pg#0)#0 != 0),
					     pg -> pg#0 + pg#1))
				);
	);
...
ZCheck(RingElement, Ideal) := Boolean => (f, I) -> (
    -- f = a polynomial in k[U]
    -- I = a zero dimensional ideal in k[U]
    -- returns if f is in the radical of I

    -- produce the matrix mf that gives multiplication by f
    B := basis ((ring I) / I);
    k := coefficientRing ring I;
    mf := substitute(contract(transpose B, f * B), k);

    -- produce the characteristic polynomial p of mf
    z := local z;
    S := k[z];
    mf = mf ** S;
    Id := S_0 * id_(S^(numgens source mf));
    p := det(Id - mf);
...
CCheck(RingElement, Ideal) := Boolean => (f, I) -> (
    -- f = a polynomial in k[U]
    -- I = an ideal in k[U]
    -- returns true if f is verified to be in the radical of I, false if it might be
